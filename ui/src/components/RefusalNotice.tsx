...
export function RefusalNotice({ data, reference, onLevel }: Props) {
  ...
  return (
    <section className="space-y-4 rounded-3xl border border-amber-500/30 bg-amber-500/5 p-6">
      ...
      <p className="text-base leading-relaxed text-slate-300">{data.explanation}</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        {suggested !== null && (
          <button
            ...
            className="rounded-xl border border-accent/40 bg-accent/20 px-4 py-2 text-base font-semibold text-indigo-100 hover:bg-accent/30"
          >
      ...
      <div>
        ...
        {showRaw && (
          <pre className="mt-2 overflow-x-auto rounded-xl border border-slate-800 bg-ink p-3 font-mono text-sm leading-relaxed whitespace-pre-wrap text-slate-400">
            {data.reason}
          </pre>
        )}
      </div>
